...
describe("getCityName (unit tests)", () => {
  ...
  it("should throw an error for invalid latitude", async () => {
    await expect(getCityName(100, -74.006)).rejects.toThrow("Invalid latitude. Expected a number between -90 and 90.");
  });

  it("should throw an error for invalid longitude", async () => {
    await expect(getCityName(40.7128, -190)).rejects.toThrow("Invalid longitude. Expected a number between -180 and 180.");
  });

  it("should throw an error when API response is missing", async () => {
    global.fetch = vi.fn().mockResolvedValue(null as any);
    await expect(getCityName(40.7128, -74.006)).rejects.toThrow("BigDataCloud API response is missing.");
  });
